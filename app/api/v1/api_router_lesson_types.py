...
@router.get("", response_model=Page[LessonType], summary="Получить все типы занятий")
...
@router.get("/{lesson_type_id}", response_model=LessonType, summary="Получить тип занятия по ID")
...
@router.put("/{lesson_type_id}", response_model=LessonType, summary="Обновить тип занятия")
...
@router.delete("/{lesson_type_id}", summary="Удалить тип занятия")
